        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <div className="rounded-3xl p-6 glass-panel flex flex-col justify-between">
            <div>
              ...
              <div className="flex p-1 bg-secondary/40 border border-border/40 rounded-2xl mb-6">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-xs sm:text-sm font-bold rounded-xl transition-all duration-200 ${
                    paymentType === "card"
              ...
              {paymentType === "card" && (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-muted-foreground font-mono font-bold px-2">
                      {currency}
                    </span>
                  </div>
                  ...
                  {submitted && (
                    <div className="mt-4 rounded-xl p-4 border border-border bg-emerald-500/10 text-sm">
                      ...
                      <div className="text-xs text-muted-foreground leading-relaxed">
                        Card checkout simulation complete. For this demonstration, your deposit
                        request has been registered as{" "}
                        <strong className="text-foreground">pending</strong>. An admin can review
                        and approve it on the admin dashboard.
                      </div>
                    </div>
                  )}
                </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4">
                        {METHODS.filter((m) => m.kind === "crypto").map((m) => (
                          <button
                            ...
                            className={`px-3 py-2.5 rounded-xl border text-xs sm:text-sm flex items-center gap-2 text-left transition ${
                              method === m.id
                    ...
                    <div className="space-y-4 rounded-2xl border border-border bg-secondary/20 p-5 mt-2">
                      <div className="flex items-center justify-between border-b border-border/60 pb-3">
                        <div>
                          ...
                          <div className="text-xs font-mono font-bold text-foreground truncate max-w-[150px]">
                            {submitted.id}
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full">
                      ...
                      {CRYPTO_INFO[submitted.method] ? (
                        <div className="flex flex-col sm:flex-row items-center gap-4 py-2">
                          <div className="p-2 bg-white rounded-xl shadow-lg shrink-0">
                            <img
                              src={`https://api.qrserver.com/v1/create-qr-code/?size=130x130&data=${encodeURIComponent(CRYPTO_INFO[submitted.method].address)}`}
                          ...
                          <div className="flex-1 w-full space-y-2">
                            <div>
                              ...
                              <span className="text-xs font-bold text-foreground">
                                {CRYPTO_INFO[submitted.method].network}
                              </span>
                            </div>
                            <div>
                              <span className="text-[10px] text-muted-foreground block">
                              ...
                              <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="text-xs font-mono bg-secondary/80 border border-border/40 px-2.5 py-1.5 rounded-lg text-foreground break-all select-all flex-1">
                                  {CRYPTO_INFO[submitted.method].address}
                                </span>
                                <button
                                  type="button"
                                  onClick={() => handleCopy(CRYPTO_INFO[submitted.method].address)}
                                  className="p-1.5 bg-primary/10 border border-primary/20 hover:bg-primary/20 rounded-lg text-primary transition shrink-0"
                                >
                                  {copied ? (
                        ...
                        <div className="text-xs text-muted-foreground py-2">
                          Address generation is processing. Please check deposit history in a few
                          minutes.
                        </div>
                      )}

                      <div className="bg-secondary/40 border border-border/40 p-3 rounded-xl text-[11px] leading-relaxed text-muted-foreground">
                        <strong className="text-foreground block mb-0.5">
                          Important Instructions:
                        </strong>
                        {CRYPTO_INFO[submitted.method]?.note ||
                          "Please transfer the requested amount to the address shown. Your request is pending admin review."}
                      </div>
                      ...
                      <div className="flex gap-2 pt-2 border-t border-border/60">
                        <button
                          ...
                          className="flex-1 h-9 rounded-lg bg-secondary/60 hover:bg-secondary text-xs font-bold transition text-foreground"
                        >
                          New Deposit
                        </button>
                        <button
                          type="button"
                          onClick={() => setPaymentType("support")}
                          className="flex-1 h-9 rounded-lg border border-primary/20 hover:bg-primary/10 text-primary text-xs font-bold transition flex items-center justify-center gap-1"
                        >
                ...
                <div className="space-y-4">
                  {!supportSubmitted ? (
                    <form onSubmit={handleSupportSubmit} className="space-y-3">
                      <div className="text-xs text-muted-foreground">
                        Need another payment option? Our billing agents support Revolut, PayPal,
                        local bank transfers, Apple/Google Pay, and custom vouchers.
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 mt-3">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full h-10 px-3 rounded-xl bg-secondary/50 border border-border text-xs font-semibold outline-none focus:ring-1 focus:ring-primary"
                          >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full h-10 px-3 rounded-xl bg-secondary/50 border border-border text-xs font-bold outline-none focus:ring-1 focus:ring-primary"
                          />
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={2}
                          className="w-full p-3 rounded-xl bg-secondary/50 border border-border text-xs outline-none focus:ring-1 focus:ring-primary resize-none placeholder:text-muted-foreground/60"
                        />
                      </div>

                      <button
                        type="submit"
                        className="w-full h-11 rounded-xl bg-gradient-to-r from-primary to-accent text-primary-foreground font-bold hover:opacity-90 transition shadow-lg shadow-primary/20 flex items-center justify-center gap-1.5 text-xs sm:text-sm"
                      >
                    ...
                    <div className="space-y-4 rounded-2xl border border-border bg-emerald-500/10 p-5 mt-2">
                      <div className="font-bold text-emerald-400 mb-1 flex items-center gap-1.5">
                        <Check className="w-4.5 h-4.5" /> Support Request Received
                      </div>
                      <div className="text-xs text-muted-foreground leading-relaxed space-y-2">
                        <p>
                          Your deposit request for{" "}
                          <strong className="text-foreground">
                            {formatMoney(Number(supportAmount), currency)}
                          </strong>{" "}
                          using <strong className="text-foreground">{supportMethod}</strong> has
                          been assigned Ticket ID{" "}
                          <strong className="text-foreground">
                            #SR-{Math.floor(10000 + Math.random() * 90000)}
                          </strong>
                          .
                        </p>
                        <p>
                          Our financial agents have been notified and will reach out to you via your
                          registered email (
                          <strong className="text-foreground">{user.email}</strong>) or live chat
                          shortly to provide manual transfer details.
                        </p>
                      </div>
                      <button
                        type="button"
                        ...
                        className="w-full h-9 rounded-lg bg-secondary hover:bg-secondary/80 text-xs font-bold text-foreground transition"
                      >
                  ...
                  <div className="border-t border-border/40 pt-4 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <a
                        ...
                        className="h-10 px-3 rounded-xl border border-border bg-secondary/40 hover:bg-secondary text-xs font-bold flex items-center justify-center gap-1.5 transition text-foreground"
                      >
                      ...
                      <a
                        href="mailto:support@norocjetx.com"
                        className="h-10 px-3 rounded-xl border border-border bg-secondary/40 hover:bg-secondary text-xs font-bold flex items-center justify-center gap-1.5 transition text-foreground"
                      >
            ...
            <div className="mt-5 text-[10px] text-muted-foreground text-center">
              Deposits are secure and encrypted. Funds are credited instantly or upon network
              confirmation.
            </div>
          </div>
          ...
          <div className="rounded-3xl p-6 glass-panel">
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto">
              ...
              {(deposits ?? []).map((d: any) => (
                <div
                  key={d.id}
                  className="flex items-center justify-between gap-3 p-3 rounded-xl bg-secondary/40"
                >
              ...
              <tbody>
                {(txs ?? []).map((t) => (
                  <tr key={t.id} className="border-t border-border/40">
                    ...
                    <td>
                      <span
                        className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${STATUS_STYLE[t.status] ?? "bg-muted"}`}
                      >
                        {t.status}
                      </span>
                    </td>
                    <td
                      className={`text-right font-mono-tabular ${Number(t.amount) >= 0 ? "text-emerald-400" : "text-red-400"}`}
                    >
                      {Number(t.amount) >= 0 ? "+" : ""}
                      {formatMoney(Number(t.amount), t.currency as never)}
                    </td>
                  </tr>
                ))}
                {(txs ?? []).length === 0 && (
